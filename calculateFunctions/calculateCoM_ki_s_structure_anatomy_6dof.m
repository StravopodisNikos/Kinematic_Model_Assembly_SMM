function [gst_anat,xi_ai_anat,M_s_com_k_i,g_s_com_k_i] = calculateCoM_ki_s_structure_anatomy_6dof(structure,assembly_parameters,anatomy,xi_pj_ref,point2figure)
% Based on
% ga_ojective_function/subroutines_executed_in_obj_fn/structure_assembly_6dof.m
...
% structure_assembly_6dof.m file ONLY for the reference anatomy! Here a
% user-specified anatomy is assembled

% It was constructed in 19.11.20 to be used for multiobjective 6dof
...
if ~(strcmp(structure(7,:),fixed_active_string_notation)) % if 7th string element is NOT active
    ...
    warning('[SMM STRUCTURE ASSEMBLY]: 7th string element is not declared ACTIVE')
     ...
     switch structure(8,:) % first switch for 1st element
        case no_passive_string_notation % case 3.1           
            ...
            switch structure(9,:)
                ...
                case passive_back_string_notation   % case 3.1.2                                
                     ...
                     [g_s_com_k_i(:,:,i_cnt,i_bodies), M_s_com_k_i(:,:,i_cnt,i_bodies)] = build_inertia_pseudomodule(g_s_m_i1_new);
                otherwise
                     warning('[SMM STRUCTURE ASSEMBLY]: 9th string element is not valid') 
            end
        ...
        otherwise
            warning('[SMM STRUCTURE ASSEMBLY]: 8th string element is not valid')
     end
...
if ~(strcmp(structure(10,:),fixed_active_string_notation)) % if 10th string element is NOT active
    ...
    warning('[SMM STRUCTURE ASSEMBLY]: 10th string element is not declared ACTIVE')
     ...
     [~,g_s_m_i1_new] = add_synthetic_joint_tf_for_ga(nDoF_string,'active_assembly',g_s_m_i1_new,assembly_parameters,active_assembly_index);
     [xi_a4_anat,g_s_m_i1_new] = build_activemodule(g_s_m_i1_new);

     i_bodies = i_bodies +1; % increased counter for body count inside metalink
     [g_s_com_k_i(:,:,i_cnt,i_bodies), M_s_com_k_i(:,:,i_cnt,i_bodies)] = build_inertia_active_static(g_s_m_i1_new);          

     i_cnt = i_cnt+1;
     xi_ai_anat(:,i_cnt) = xi_a4_anat;
     %% END - Add active DXL
     i_bodies = 0;
...
if ~(strcmp(structure(13,:),fixed_active_string_notation)) % if 10th string element is NOT active
    ...
    warning('[SMM STRUCTURE ASSEMBLY]: 13th string element is not declared ACTIVE')
     ...
     [~,g_s_m_i1_new] = add_synthetic_joint_tf_for_ga(nDoF_string,'active_assembly',g_s_m_i1_new,assembly_parameters,active_assembly_index);
     [xi_a5_anat,g_s_m_i1_new] = build_activemodule(g_s_m_i1_new);

     i_bodies = i_bodies +1; % increased counter for body count inside metalink
     [g_s_com_k_i(:,:,i_cnt,i_bodies), M_s_com_k_i(:,:,i_cnt,i_bodies)] = build_inertia_active_static(g_s_m_i1_new);          

     i_cnt = i_cnt+1;
     xi_ai_anat(:,i_cnt) = xi_a5_anat;
     %% END - Add active DXL
     i_bodies = 0;
...
if ~(strcmp(structure(16,:),fixed_active_string_notation)) % if 7th string element is NOT active
    wrong_string_structure = true;
    warning('[SMM STRUCTURE ASSEMBLY]: 16th string element is not declared ACTIVE')
else

     %% START - Add active DXL
     active_assembly_index = 6;
     [~,g_s_m_i1_new] = add_synthetic_joint_tf_for_ga(nDoF_string,'active_assembly',g_s_m_i1_new,assembly_parameters,active_assembly_index);
     [xi_a6_anat,g_s_m_i1_new] = build_activemodule(g_s_m_i1_new);

     i_bodies = i_bodies +1; % increased counter for body count inside metalink
     [g_s_com_k_i(:,:,i_cnt,i_bodies), M_s_com_k_i(:,:,i_cnt,i_bodies)] = build_inertia_active_static(g_s_m_i1_new);          

     i_cnt = i_cnt+1;
     xi_ai_anat(:,i_cnt) = xi_a6_anat;
