...
%     tspan = [time_samples(task_point_cnt-1) time_samples(task_point_cnt)];
    tspan = [time_samples_trapz(task_point_cnt-1) time_samples_trapz(task_point_cnt)];
    tspan_discrete_ode = linspace(time_samples_trapz(task_point_cnt-1),time_samples_trapz(task_point_cnt),10);
    ...
    for ode_cnt=1:size(t,1)
        [torque(ode_cnt,:),V(ode_cnt,:),error(ode_cnt,:)] = postODEoutput_PID_computed_torque_control_3DoF_MMD(x(ode_cnt,:)',x_d,ddq_d(:,task_point_cnt),dt,s_for_compute_Mij_429_3DoF,s_pid,Vold);
        Vold = V(ode_cnt,:);
    end
...
%     plot(t,error(:,1)) % only for visual tuning of pid gains 
    ...
    tx_torque_er_new = horzcat(t,x,torque,error);
