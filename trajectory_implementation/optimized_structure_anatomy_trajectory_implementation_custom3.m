function optimized_structure_anatomy_trajectory_implementation_custom3(structure,assembly_parameters,isotropic_anatomy_extracted,way_pts,time_pts,s_pd,plot_title)
...
% ** INVERSE DYNAMICS PD CONTROL IMPLEMENTATION

% This file was developed from: optimized_structure_anatomy_trajectory_implementation_custom.m 
...
urdf_files_structures_path_relative_to_matlab_ws = fullfile('matlab_ws','Kinematic_Model_Assembly_SMM','trajectory_implementation','urdf_files_structures_anatomies_here',filesep);
urdf_files_structures_library_path = strcat(root_path,urdf_files_structures_path_relative_to_matlab_ws); addpath(urdf_files_structures_library_path);
%% URDF ROBOT MODEL OF IMPORTED STRUCTURE
robotURDFname = 's0_rand_s_rand_anat.urdf';
robotURDFfile = strcat(urdf_files_structures_library_path,robotURDFname);
[TestRobot,~,~,~] = ImportRobotRefAnatomyModel(robotURDFfile);
...
% [q_d,dq_d,ddq_d,time_samples_trapz,~] = trapveltraj(way_pts,size(time_samples,2));
%% EXECUTION - USING ODE
ode_calc_points    = 20;
...
%         epsilon0= last_epsilon';
    ...
    field1 = 'q_d';       value1 = q_d(:,task_point_cnt);
    field2 = 'dq_d';      value2 = dq_d(:,task_point_cnt);
    field3 = 'ddq_d';     value3 = ddq_d(:,task_point_cnt);
    s_d    = struct(field1,value1,field2,value2,field3,value3);
    
    % build the error state
%     epsilon = epsilon0;
    e  = s_d.q_d - x0(1:3)';
    de = s_d.dq_d - x0(4:6)';
    e_state0 = vertcat(e,de);   
    
    % solve the ode
    tic;
    ode_derivative_fn = @(t,e_state) PD_inverse_dynamics_control_3DoF_MMD(t,e_state,s_d,s_pd,s_for_compute_Mij_429_3DoF,dt,TestRobot);
%     ode_output_fn = @OutputFcn_PD_inverse_dynamics_control_3DoF_MMD;
    ode_opts = odeset('Mass',@(t,e_state) MassMatrixMatlab(e_state,s_d,s_for_compute_Mij_429_3DoF,dt,TestRobot),'InitialStep',1e-3,'MaxStep',1e-2,'RelTol',1e-5,'AbsTol',1e-7);
    [t,e_state] = ode23t(ode_derivative_fn, tspan_discrete_ode, e_state0,ode_opts);
    disp('finished ode');
    toc;
    ...
    x_d(1:3) = s_d.q_d;
    x_d(4:6) = s_d.dq_d;
    x_c      = zeros(size(t,1),6);
    for ode_cnt=1:size(t,1)
%         e_c(ode_cnt,:) = e_state(ode_cnt,1:3);
        x_c(ode_cnt,:) = x_d - e_state(ode_cnt,1:6);
    end
% %     figure;plot(t,x_c(:,1)); figure;plot(t,x_c(:,2)); figure;plot(t,x_c(:,3));
    ...
    for ode_cnt=1:size(t,1)
        [torque(ode_cnt,:),Vold,error(ode_cnt,:)] = postODEoutput_PD_inverse_dynamics_control_3DoF_MMD(e_state(ode_cnt,:)',s_d,s_pd,s_for_compute_Mij_429_3DoF,dt,Vold);
    end
    
%     plot(t,error(:,1)) % only for visual tuning of pid gains 
